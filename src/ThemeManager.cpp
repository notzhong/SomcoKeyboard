...
ThemeManager::~ThemeManager() {
    qDeleteAll(m_availableThemes);
}

QQmlListProperty<KeyboardTheme> ThemeManager::availableThemes() {
    return QQmlListProperty<KeyboardTheme>(this, this,
                                           [](QQmlListProperty<KeyboardTheme> *list, KeyboardTheme *theme) {
                                               auto self = static_cast<ThemeManager*>(list->data);
                                               self->addTheme(theme);
                                           },
                                           [](QQmlListProperty<KeyboardTheme> *list) -> qsizetype {
                                               return static_cast<ThemeManager*>(list->data)->m_availableThemes.count();
                                           },
